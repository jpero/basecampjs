/*!
 * A Basecamp API wrapper in JavaScript (https://github.com/trekdemo/basecampjs)
 *
 * Copyright (c) 2008 Brandon Aaron (brandonaaron.net)
 * Copyright (c) 2011 Gergő Sulymosi (gergo.sulymosi.com)
...
function Basecamp( url, api_token ) {

  this.post = function( path, callbacks, requestXML ) {
    request( "POST", path, callbacks, requestXML );
  };
  this.get = function( path, callbacks, requestXML ) {
    request( "GET", path, callbacks, requestXML)
  }
  this.put = function( path, callbacks, requestXML ) {
    request( "PUT", path, callbacks, requestXML)
  }
  this.delete = function( path, callbacks, requestXML ) {
    request( "DELETE", path, callbacks, requestXML)
  }
  ...
  function connect(path, type) {
    ...
    type = type || "GET"
    connection.open( type, url + path, true, api_token, 'X' );
    return connection;
  };
   ...
   * headers = {}
   */
  function request(type, path, callbacks, body, headers) {
    var connection = connect( path, type );
